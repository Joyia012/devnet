...
function initRPC() {
  ...
  * @param {String} address
		...
		* @return {String} status
  */
 server.expose('sendassettoaddress', function(args, opt, cb) {
   if (args.length != 3) {
     return cb("wrong parameters");
   }
   ...
   let toAddress = args[1];
   let amount = args[2];
   if (amount && toAddress && asset) {
     if (validationUtils.isValidAddress(toAddress))
         headlessWallet.readSingleAddress(function(fromAddress) {
             createDivisibleAssetPayment(asset, amount, fromAddress, toAddress, function(err, unit) {
                 cb(err, err ? undefined : unit);
             });
         });
     else
...
function createIndivisibleAssetPayment(asset, amount, fromAddress, toAddress, callback) {
...
function createDivisibleAssetPayment(asset, amount, fromAddress, toAddress, callback) {
    var network = require('byteballcore/network.js');
